...
import net.sf.opensmus.LVoid;
import net.sf.opensmus.MUSErrorCode;
import net.sf.opensmus.MUSLog;
import net.sf.opensmus.MUSMessage;
import net.sf.opensmus.MUSMsgHeaderString;
import net.sf.opensmus.MUSMsgHeaderStringList;
import net.sf.opensmus.MUSUser;

public class FloodFilter extends SimpleChannelInboundHandler<ByteBuf> {
  ...
  protected void channelRead0(ChannelHandlerContext ctx, ByteBuf inBuffer) throws Exception {
    ...
    if (strLen < 0 || strLen > inBuffer.readableBytes()) {
      MUSLog.Log("Floodfilter MUSMsgHeaderString size error : " + strLen + " " + inBuffer.readableBytes() + " " + ByteBufUtil.hexDump(inBuffer), MUSLog.kDeb);
    ...
    super.channelRead(ctx, inBuffer);
  }
